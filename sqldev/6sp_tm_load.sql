...
-- VERSION TO UPDATE TEAM CITY WITHOUT DELETING
create or replace procedure lg.sp_team_update()
language plpgsql
as $$
begin
    update lg.team set team_city=null, team_shrt=null;
    insert into lg.team (
        lg_id, team_id, team, team_cde, team_long, team_city, team_shrt)
        select
            0,
        ...
        group by a.season_id, a.team_id, a.team_abbreviation, 
            b.team_code, a.team_name, b.team_city, b.team_name
        order by a.season_id desc
    on conflict (team_id) do update set team_city = excluded.team_city, team_shrt = excluded.team_shrt;
